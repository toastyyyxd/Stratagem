...
pub fn reserveVirtualRegion(size: usize) !*u8 {
    ...
    switch (builtin.target.os.tag) {
        ...
        .windows => {
            const alloc_type = std.os.windows.MEM_RESERVE;
            const flProtect = std.os.windows.PAGE_READWRITE;
            const ptr = std.os.windows.VirtualAlloc(null, size, alloc_type, flProtect);
            if (ptr == null) {
                return error.VirtualMemoryReservationFailed;
            }
            return @ptrCast(ptr);
...
pub fn releaseVirtualRegion(ptr: *u8, size: usize) !void {
    ...
    switch (builtin.target.os.tag) {
        ...
        .windows => {
            const result = std.os.windows.VirtualFree(@ptrCast(ptr), 0, std.os.windows.MEM_RELEASE);
            if (result == 0) {
                return error.VirtualMemoryReleaseFailed;
            }
        },
...
pub fn commitVirtualPages(ptr: *u8, size: usize) !void {
    switch (builtin.target.os.tag) {
        .linux => {
            const result = std.os.linux.mprotect(@ptrCast(ptr), size, std.os.linux.PROT.READ | std.os.linux.PROT.WRITE);
            if (result != 0) {
        ...
        .windows => {
            const result = std.os.windows.VirtualAlloc(
                ptr,
                size,
...
pub fn decommitVirtualPages(ptr: *u8, size: usize) !void {
    switch (builtin.target.os.tag) {
        ...
        .windows => {
            const result = std.os.windows.VirtualFree(@ptrCast(ptr), size, std.os.windows.MEM_DECOMMIT);
            if (result == 0) {
                return error.VirtualMemoryDecommitFailed;
            }
            return;
